$(document).ready(function () {
    ...
    $('#add-exp-btn').click( function( event ) {
        ...
        if (validateExp('add')) {
            let experience = {
                'title' :  $('#exp-title').val(),
                'company_name' : $('#exp-company').val(),
                'location' : $('#exp-location').val(),
                'employment_type' : $('#exp-type').val(),
    ...
    $('#add-cert-card').click( function( event ) {
        event.preventDefault();
        if (validateCert('add')) {
            var certification = {
                'name' :  $('#cert-name').val(),
    ...
    function validateCert(action) {
        var all_input = document.querySelectorAll(`#${action}-certification input`),
            ...
            check = true;
        for (cert_input of all_input) {
            if (cert_input.id == "cert-credential-id") {
                continue;
            }
            if (cert_input.value.length <= 1) {
                cert_input.classList.add("warning");
                check = false;
            } else {
                cert_input.classList.remove("warning");
                if (cert_input.id == "cert-issue-date") {
                    check_issue_date = cert_input;
                }
                if (cert_input.id == "cert-expiration-date") {
                    check_expiration_date = cert_input;
                }
